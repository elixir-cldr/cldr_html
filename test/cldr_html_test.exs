  ...
  describe "currency_select/3" do
    test "with selected currency" do
      assert safe_to_string(
        Cldr.HTML.Currency.currency_select(:my_form, :currency, selected: :USD, currencies: ~w(USD EUR JPY COP))
    ...
    test "without selected currency" do
      assert safe_to_string(
        Cldr.HTML.Currency.currency_select(:my_form, :currency, currencies: ~w(USD EUR JPY COP))
    ...
    test "when selected currency is not in currencies" do
      assert safe_to_string(
        Cldr.HTML.Currency.currency_select(:my_form, :currency, selected: :USD, currencies: ~w(EUR JPY))
        ) ==  ~s(<select id="my_form_currency" name="my_form[currency]" selected="USD">) <>
              ~s(<option value="EUR">EUR - Euro</option>) <>
              ~s(<option value="JPY">JPY - Japanese Yen</option>) <>
              ~s(<option value="USD">USD - US Dollar</option>) <> ~s(</select>)
    ...
    test "with thai locale" do
      assert safe_to_string(
        Cldr.HTML.Currency.currency_select(:my_form, :currency, currencies: ~w(USD EUR JPY COP), locale: "th")
      ) ==  ~s(<select id="my_form_currency" name="my_form[currency]">) <>
    ...
    test "with invalid selected" do
      assert Cldr.HTML.Currency.currency_select(:my_form, :currency, selected: ~w(invalid1), currencies: ~w(USD EUR JPY COP))
        == {:error, {Cldr.UnknownCurrencyError, "The currency [\"invalid1\"] is invalid"}}

    end

    test "with invalid currencies" do
      assert Cldr.HTML.Currency.currency_select(:my_form, :currency, currencies: ~w(invalid1 invalid2))
        == {:error, {Cldr.UnknownCurrencyError, "The currency \"invalid1\" is invalid"}}
    end
